                                ...
                                return (
                                  <div key={draft.id} className="bg-neutral-50 dark:bg-neutral-800 rounded-lg p-3 text-xs group">
                                    <div className="flex items-center gap-2 mb-2">
                                      ...
                                      {deleteDraftConfirmId === draft.id ? (
                                        <span className="inline-flex items-center gap-1">
                                          <button onClick={async () => { await fetch(`/api/leads/draft/${draft.id}`, { method: "DELETE" }); setDeleteDraftConfirmId(null); if (expandedId) { setLeadDrafts((prev) => ({ ...prev, [expandedId]: (prev[expandedId] || []).filter((d) => d.id !== draft.id) })); } }} className="text-[10px] font-medium text-white bg-red-500 hover:bg-red-600 px-2 py-0.5 rounded transition-colors duration-150 cursor-pointer">Delete</button>
                                    ...
                                    <div className="flex items-center gap-1.5 mt-2">
                                      {editingDraftId === draft.id ? (
                                        <>
                                          <button onClick={async () => { await fetch(`/api/leads/draft/${draft.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: editingDraftContent }) }); setLeadDrafts((prev) => ({ ...prev, [lead.id]: (prev[lead.id] || []).map((d) => d.id === draft.id ? { ...d, content: editingDraftContent } : d) })); setEditingDraftId(null); toast("Draft updated"); }} className="px-2 py-1 text-xs font-medium text-white bg-emerald-600 dark:bg-emerald-500 hover:bg-emerald-700 dark:hover:bg-emerald-600 rounded-md border border-emerald-600 dark:border-emerald-500 transition-all duration-150 cursor-pointer">Save</button>
                                          <button onClick={() => setEditingDraftId(null)} className="px-2 py-1 text-xs text-neutral-500 dark:text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 border border-neutral-200 dark:border-neutral-700 rounded-md transition-all duration-150 cursor-pointer">Cancel</button>
                                        </>
                                      ) : (
                                        <>
                                    ...
                                    <div className="flex items-center gap-2 mb-1.5">
                                      ...
                                      {deleteConfirmId === msg.id ? (
                                        <span className="inline-flex items-center gap-1 ml-1">
                                          <button onClick={async () => { await fetch(`/api/leads/messages/${msg.id}`, { method: "DELETE" }); setDeleteConfirmId(null); setDeletedId(msg.id); setTimeout(() => setDeletedId(null), 1500); if (expandedId) { setLeadMessages((prev) => ({ ...prev, [expandedId]: (prev[expandedId] || []).filter((m) => m.id !== msg.id) })); } }} className="text-[10px] font-medium text-white bg-red-500 hover:bg-red-600 px-2 py-0.5 rounded transition-colors duration-150 cursor-pointer">Delete</button>
